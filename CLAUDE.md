...
- `CHANGELOG.md` is auto-generated by `standard-version` -- it is the source of truth for what shipped in which version
- `documents/FEATURE_SPEC.md` and `documents/ROADMAP.md` are manually maintained and may drift from CHANGELOG after releases
- When updating docs: cross-reference `git log`, `CHANGELOG.md`, and `package.json` version before editing `documents/`
- Roadmap version planning (future versions) may not match actual releases -- renumber planned items when updating
